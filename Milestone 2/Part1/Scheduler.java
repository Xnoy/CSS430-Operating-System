...
public class Scheduler extends Thread
{
	...
	//this.setPriority( 6 ); //remove from part 1
	...
	while ( true ) {
		...
		if ( current != null ) {
		    if ( current.isAlive( ) )
			current.resume( ); //current.setPriority( 4 ); //replace from part 1
		    else {
			// Spawn must be controlled by Scheduler
			// Scheduler must start a new thread
			current.start( ); 
			//current.setPriority( 4 ); //remove from part 1
		    }
		}
		...
		synchronized ( queue ) {
			...
			current.suspend( ); //current.setPriority( 2 ); //replace part 1
